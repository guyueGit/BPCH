...
#from utils.args_adapter import threshold
from sklearn.metrics.pairwise import euclidean_distances
#from utils.get_args import get_args
from utils.args_adapter import get_args
...
class HyP(torch.nn.Module): 
    ...
    def forward(self, x=None, y=None, label=None):
        ...
        pos = 1 - cos
        neg = F.relu(cos - self.args.threshold)

        cos_t = F.normalize(y, p=2, dim=1).mm(F.normalize(self.proxies, p=2, dim=1).T)
        pos_t = 1 - cos_t
        neg_t = F.relu(cos_t - self.args.threshold)
        ...
        if self.args.alpha > 0:
            ...
            if len((cos_sim == 0).nonzero()) == 0:
                ...
                neg = self.args.alpha * F.relu(x_sim - self.args.threshold)
                neg_t = self.args.alpha * F.relu(t_sim - self.args.threshold)
                neg_xt = self.args.alpha * F.relu(xt_sim - self.args.threshold)
