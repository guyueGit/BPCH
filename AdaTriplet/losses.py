...
def bit_var_loss():#计算二进制哈希码的 ​​位方差损失​​，用于正则化哈希码分布。
    def F(x):
...
class LowerBoundLoss(torch.nn.Module):#计算 ​下限约束损失​​，强制模型输出不低于0
    def __init__(self):
...
class TripletCustomMarginLoss(TripletMarginLoss):#改进的 ​​三元组边际损失​​，适配多标签（Multi-label）场景，支持动态参考样本和自定义三元组生成策略
    """
    Args:
